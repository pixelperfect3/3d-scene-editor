...
void vrpn_WiiMote::initialize_wiimote_state(void)
{
  ...
  // Turn on IR sensing
  wiiuse_set_ir(wiimote->device, 0); //TODO : disable IR sensing for our app.
}
        ...
        wiimote(new vrpn_Wiimote_Device)
{
	...
	vrpn_Analog_Output::o_num_channel = VRPN_WIIMOTE_NUM_CHANNELS;
	...
	vrpn_HANDLERPARAM p)
{
	...
	me->handle_change_request(chan_num, value);
    return 0;
}
    ...
    vrpn_HANDLERPARAM p)
{
	...
	//update change requests (for each channel message):
	for (vrpn_int32 ii = 0; ii < num; ii++) {
		vrpn_float64 value;
		vrpn_unbuffer(&bufptr, &value);
		me->handle_change_request(ii, value);
	}
	//set the rest to 0:
	for (vrpn_int32 ii = num; ii < me->o_num_channel; ii++) {
		vrpn_float64 value = 0;
		me->handle_change_request(ii, value);
	}
    return 0;
}

void vrpn_WiiMote::handle_change_request(vrpn_int32 channel, vrpn_float64 value) {
	...
	case VRPN_WIIMOTE_CHANNEL_LED_1:
		wiiuse_set_leds(wiimote->device, WIIMOTE_LED_1);
		break;
	case VRPN_WIIMOTE_CHANNEL_LED_2:
		wiiuse_set_leds(wiimote->device, WIIMOTE_LED_2);
		break;
	case VRPN_WIIMOTE_CHANNEL_LED_3:
		wiiuse_set_leds(wiimote->device, WIIMOTE_LED_3);
		break;
	case VRPN_WIIMOTE_CHANNEL_LED_4:
		wiiuse_set_leds(wiimote->device, WIIMOTE_LED_4);
		break;
	}
}
